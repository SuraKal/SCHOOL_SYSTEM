  ...
  if (isset($_GET['Id']) && isset($_GET['action']) && $_GET['action'] == "edit")
    {
                ...
                where Id='$Id'");
                        ...
                        {
                            $status_msg = "<div class='alert alert-danger' style='margin-right:700px;'>An error Occurred!</div>";
                        }
    }
  ...
  if (isset($_GET['Id']) && isset($_GET['classArmId']) && isset($_GET['action']) && $_GET['action'] == "delete")
	{
        ...
        $query = mysqli_query($conn,"DELETE FROM tblreport WHERE Id='$Id'");
          if ($query == TRUE) {
              $status_msg = "<div class='alert alert-success' style='margin-left:50px;margin-right:700px;'>Report deleted</div>";
          }else{
                    ...
                    <div class="table-responsive p-3">
                      <table class="table align-items-center table-flush table-hover" id="dataTableHover table_width">
                        ...
                        <tbody>
                          ...
                          <?php
                              $query = "SELECT *,tblreport.Id,tblreport.classArmId,tblclass.className,tblclassarms.classArmName
                              FROM tblreport
                              INNER JOIN tblclass ON tblclass.Id = tblreport.classId
                              INNER JOIN tblclassarms ON tblclassarms.Id = tblreport.classArmId
                              WHERE tblreport.teacherId = '$teacherId' AND tblreport.staff = '2'";
                              $rs = $conn->query($query);
                              $num = $rs->num_rows;
                              $sn=0;
                              $status="";
                                ...
                                while ($rows = $rs->fetch_assoc())
                                  {
                                    if($rows['status'] == '0'){
                                      $status = "On-Progress";
                                      $handlebtn = "<a href='?action=edit&Id=".$rows['Id']."' class
